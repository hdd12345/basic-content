...
//            		  list.add(stack.pop());
//              }
              while(last != '(' && priority(last) >= priority(ch)){
            	  list.add(stack.pop());
            	  last = stack.peek().charAt(0);
              }
      ...
      if(nFlag == 1) list.set(0, "-"+list.get(0));
      return list;  
  }  
  ...
  public static void main(String[] args) {  
      String expression = "9-2+6-3*2+5#";  
      try {  
          System.out.println(Eval.toSuffixSequence(expression).toString());  
